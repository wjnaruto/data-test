...
The API supports all of the following request shapes under the same tenant scope:

1. dataset changes only
2. attribute changes only
3. dataset changes and attribute changes together

## 2. OAuth2 and Token Validation

### Expected flow

The auth server design for this service is:
...
3. The frontend exchanges the code for tokens at `/oauth2/token`.
4. The frontend calls the submit API with:

   `Authorization: Bearer <access_token>`

5. The submit API validates the JWT access token locally using JWKS.
6. The submit API extracts from JWT claims:
   - user identity
   - display name
   - tenant AD group / role information
7. The submit API compares the user groups from token claims with the target tenant requester group in `tenant_role_mapping`.
...
This service validates JWT access tokens locally with:
...
The current implementation expects JWT validation configuration:

| Setting | Required | Purpose |
| --- | --- | --- |
| `AUTH_JWKS_URL` | Yes | JWKS endpoint used to verify JWT access token signature |
| `AUTH_ISSUER` | Yes | expected JWT issuer |
...
JWT validation is the only supported validation path for this service.
...
```json
{
  ...
  "datasets": [
    {
      "clientRef": "ds-new-001",
      "action": "A",
      "tableMetadata": {
        "tableName": "customer_positions",
        "domainId": "domain-001",
        "tenantUniqueId": "tenant-001",
        "tableDescription": "New dataset"
      }
    },
    {
      "action": "U",
...
- all items in one submit must belong to the same request-level `tenantUniqueId`
- all items in one submit must belong to the same request-level `domainId`
- the API accepts:
  - `datasets` only
  - `attributes` only
  - `datasets` and `attributes` together

### Dataset rules

- `datasets` is a list
- each dataset item supports `A / U / D`
- `entityId` is required for `U` and `D`
- `tableMetadata` is required
- for `A`, a new dataset id is generated by backend
- for `A`, `clientRef` is optional but recommended if new attributes in the same request need to point to the new dataset
...
- for `A`, the target dataset can be provided by:
  - `metadata.tableId` for an existing dataset
  - `datasetClientRef` for a new dataset added in the same request
- if `datasetClientRef` is used, it must match a dataset add item in `datasets`
...
- if an attribute points to an existing dataset, backend validates that the target dataset belongs to the same tenant as the request
...
2. resolve user claims from JWT access token
...
- one request may create zero, one, or many `table_entity_pending` rows
- one request may create zero, one, or many `attribute_entity_pending` rows
- `approval_request` remains one row per submit action
...
The auth server guide for this project explicitly supports:

- Authorization Code flow
- PKCE for public clients
- JWT access tokens
- JWKS endpoint for signature verification
- custom `groups` claim inside JWTs

This service therefore does not need userinfo or introspection for submit authentication.
