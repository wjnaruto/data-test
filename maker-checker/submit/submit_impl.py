...
from db import submit_queries
from models.submit_models import (
    AttributeSubmitItem,
    DatasetSubmitItem,
    SubmitConflictItem,
    SubmitRequest,
...
class SubmitService:
    ...
    async def submit(self, payload: SubmitRequest, user: AuthenticatedUser) -> SubmitResponse:
        ...
        async with db.pool.acquire() as connection:
            async with connection.transaction():
                await validate_requester_tenant_access(connection, payload.tenantUniqueId, user)
                ...
                await submit_queries.insert_approval_request(
    ...
    async def _stage_dataset(
        ...
        await submit_queries.insert_table_pending(
            connection=connection,
    ...
    async def _stage_attribute(
        ...
        await submit_queries.insert_attribute_pending(
            connection=connection,
            pending_id=str(uuid4()),
            request_id=request_id,
