...
class TableEntityPending(SQLModel, table=True):
    ...
    target_table_id: Optional[str] = Field(default=None, foreign_key="table_entity.id", max_length=36)
    table_metadata: dict = Field(default_factory=dict, sa_column=sa.Column(JSONB, nullable=False))
    table_name: Optional[str] = Field(default=None, sa_column=sa.Column(sa.String(length=256), sa.Computed("(table_metadata ->> 'tableName')", persisted=True), nullable=True))
    tenant_name: Optional[str] = Field(default=None, sa_column=sa.Column(sa.String(length=256), sa.Computed("(table_metadata ->> 'tenantName')", persisted=True), nullable=True))
    updatedat: Optional[int] = Field(default=None, sa_column=sa.Column(sa.BigInteger(), sa.Computed("((table_metadata ->> 'updatedAt')::bigint)", persisted=True), nullable=True))
    updatedby: Optional[str] = Field(default=None, sa_column=sa.Column(sa.String(length=256), sa.Computed("(table_metadata ->> 'updatedBy')", persisted=True), nullable=True))
    deleted: Optional[bool] = Field(default=None, sa_column=sa.Column(sa.Boolean(), sa.Computed("((table_metadata ->> 'deleted')::boolean)", persisted=True), nullable=True))
    createdat: Optional[int] = Field(default=None, sa_column=sa.Column(sa.BigInteger(), sa.Computed("((table_metadata ->> 'createdAt')::bigint)", persisted=True), nullable=True))
    attributes_metadata: Optional[dict] = Field(default=None, sa_column=sa.Column(JSONB, sa.Computed("((table_metadata ->> 'attributesMetadata')::jsonb)", persisted=True), nullable=True))
    domain_id: Optional[str] = Field(default=None, sa_column=sa.Column(sa.String(length=36), sa.Computed("(table_metadata ->> 'domainId')", persisted=True), nullable=True))
    tenant_unique_id: Optional[str] = Field(default=None, sa_column=sa.Column(sa.String(length=36), sa.Computed("(table_metadata ->> 'tenantUniqueId')", persisted=True), nullable=True))
    table_metadata_text: Optional[str] = Field(default=None, sa_column=sa.Column(sa.Text(), sa.Computed("(table_metadata::text)", persisted=True), nullable=True))
